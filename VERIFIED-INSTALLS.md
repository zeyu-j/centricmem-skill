...
|---|---|---|
| DeepSeek Harness (dsh) | `npx skills add zeyu-j/centricmem-skill` (it reads `~/.agents/skills` directly) | `dsh-skill-filesystem` reads that path; its `dsh-hooks-claude-code` bridge runs this package's `hooks/hooks.json` with `pluginRoot` set. The plugin command itself needs a TTY. |
