...
| **goose** 1.51.0 | `goose/*.yaml` recipes and `goose/centricmem-ambient.mjs` | The refresher writes `status=OK` with a credential and `status=NO-KEY` without one, and disclaims both. goose has no hook surface, so the recipe is the whole optimisation |
...
| **Qwen Code** 0.24.4 | `qwen/` - a `SessionStart` hook that prints Qwen's own shape (`hookSpecificOutput.additionalContext`) plus a `settings-hooks.example.json` and an extension manifest | The hook returns real context in Qwen's shape when a credential is present and nothing when it is not; the smoke job runs it on Linux and macOS with no credential, asserting silence and exit 0 |
...
(`tools/ambient.mjs`) with thin wrappers around it (`hermes/` carries its own copy, because Hermes runs its
hook from outside the package).
