...
| **goose** 1.52.0 | `goose/*.yaml` recipes, `goose/centricmem-ambient.mjs`, and the plugin's `hooks/hooks.json` | The refresher writes `status=OK` with a credential and `status=NO-KEY` without one, and disclaims both. goose **does** have a hook surface through plugins: `SessionStart` and `SessionEnd` run `hooks/ambient.mjs` and `hooks/close.mjs`. On goose a hook's stdout is the decision channel and carries no context, so the ambient half writes the file named by `GOOSE_MOIM_MESSAGE_FILE` instead of printing - the built-in **Top Of Mind** extension is what injects it, and it must be enabled. The recipes are installed rather than merely shipped: `goose plugin install` copies skills and hooks only, so the `SessionStart` entry runs `goose/wire-recipes.mjs`, which writes both YAML into `~/.config/goose/recipes/` and never replaces a file that does not carry its marker. Building it settled three facts - 1.52.0 searches `~/.config/goose/recipes/` and also `~/.agents/recipes/`, with `goose recipe list` naming the installed files; and `GOOSE_RECIPE_PATH` **cannot** be supplied through `config.yaml`, where the key is ignored and a directory named there stays invisible. That is why an environment variable was not good enough on a host launched from the dock |
...
hook too, which is the assertion that would have caught the release where that hook could not even load. The recipe wiring has its own job on a fake home, because the contract has two halves to prove - a goose gets the files, and a host that merely sets `PLUGIN_ROOT` gets nothing - and the goose-install job then has a real goose list what the wiring installed. The
