...
- Skill refresh: update the copy **this host loads**. `npx skills add -g` writes `~/.agents` and does **not** update Reasonix's plugin tree. `cm_ambient` with `shelf=` shapes Recent/Session. Missing `charter` on a library row means unset (fall back to displayName). Librarian CLI unchanged (**0.21.76**).
...
- Reach clarifies **remote/cloud is not paste-key-only**: OAuth when this agent receives the browser login; Bearer only when it cannot; sandbox claim EPERM â†’ human claim outside / OAuth / paste in settings. Librarian CLI unchanged (**0.21.76**).
...
- Card **writing style** is the human + this agent Ã¢â‚¬â€ CentricMem only requires summary + key points (structure). Different agents on the same shelf write differently on purpose. Librarian CLI unchanged (**0.21.75**).
...
- Define **chunk** (accepted unit this reply) and **yield** (any reply end). Resume after context compress / checkpoint / new chat = new session: `cm_health` Ã¢â€ â€™ `cm_ambient` first. Single-source Ã‚Â§4 sweep + gate (Non-Micro + zero `cm_*` + no Don't log Ã¢â€ â€™ sweep before yield). Adaptive vs wrap-up table. Slim SKILL; connect recipes stay in REFERENCE. CLI unchanged (**0.21.74**).
...
- Shelf routing (#0180): user explicit Ã¢â€ â€™ charter/displayName match Ã¢â€ â€™ path tiebreak only Ã¢â€ â€™ mint with human-supplied name. Cite the matched line before write. Per-shelf charter shipped in Skill **0.21.76** / librarian **>=0.21.75**.
...
- Codex / ChatGPT Mixed is not Ã¢â‚¬Å“OAuth plus paste a keyÃ¢â‚¬Â. Mixed tries unauthenticated `initialize` / `tools/list`; we 401 that. Pick OAuth for the browser, or Bearer in `http_headers` instead of OAuth.
...
- First connect is signup + `/connect?device=` this turn. Already added is only a Bearer or a finished OAuth login in this agent Ã¢â‚¬â€ URL-only plugin / `mcp add` is first connect. Do not skip the device URL because `mcp login` exists. Loopback `127.0.0.1` does not authenticate a cloud or remote agent.
- Librarian **>=0.21.51**. CLI **>=0.21.50**.
...
- Daily cards: `cm_import` `{items}` (a `bundle` that is `{items:[...]}` with no `version` is the same path). ImportBundle is `{version:1, lessons?, imported?, Ã¢â‚¬Â¦}` Ã¢â‚¬â€ junk shapes name that contract. `skipExisting` skips a dest slug instead of `slug-2.md`.
- `cm_log_decision` `refs`: `1`, `0001`, `#0001`, or a comma list. Junk is 400.
- Copy asides `imported/kept/from-*` stay out of FTS (`cm_show` by path still works). Keep stubs Ã¥ÂÂ¦Ã¥Â­Ëœ on collision. `limit` caps browse and tags-only. Omit `cm_library` `id` to list shelves. `cm_index` reports `skipped`. `cm_move` `files=` is whole Markdown paths only; companion keep stubs and `- **Shelf**:` follow the card.
...
- `cm_delete` `{file, shelf, heading}` removes one `##` section in `lessons.md` (notes stay sections, not one file per card). Do not pass `file#heading`. Same-title `cm_note` is an error Ã¢â‚¬â€ pick a new title, or delete that heading then rewrite. Import `dryRun` `files[].file` is the disk path apply will write.
- CLI **>=0.21.48**.

## 0.21.62

- If they already added a key or MCP URL in this agent, `cm_*` missing means a **new chat** Ã¢â‚¬â€ do not mint `/connect?device=` and do not tell them to add the URL with no Bearer (that drops the key). Ignore leftover `centricmem` on PATH when `connect --device` is missing (npm 0.14.x). First connect is unchanged.
- CLI **>=0.21.36**.
...
- Pi: `pi install https://github.com/zeyu-j/centricmem-skill` (package.json `pi.skills`). MCP is URL-only in `~/.pi/agent/mcp.json` Ã¢â‚¬â€ not auto-wired.
...
- After plugin install, if `cm_*` are missing the same reply must send https://centricmem.com/login?signup=1 **and** a `/connect?device=` URL. Plugin `mcp.json` is URL-only Ã¢â‚¬â€ it is not a completed connect.
...
- After Skill install, the same reply must still send https://centricmem.com/login?signup=1 and tell them to save a backup of the key Ã¢â‚¬â€ Keys shows it only once; never paste it in chat.
- CLI **>=0.21.36**.

## 0.21.51

- After Skill install, the same reply must send https://centricmem.com/login?signup=1 **and** a `/connect?device=` URL, **and** tell them to save a backup of the key Ã¢â‚¬â€ Keys shows it only once; never paste it in chat.
- CLI **>=0.21.36**.

## 0.21.50

- After Skill install, the same reply must send https://centricmem.com/login?signup=1 **and** a `/connect?device=` URL. Agents with a shell (Hermes) POST `https://mem.centricmem.com/connect/device` to mint that session; keep the secret off chat; claim and write Bearer to this agentÃ¢â‚¬â„¢s MCP (`~/.hermes/config.yaml`). Signup-only stays for blocked shells (Grok). `connect --device` stays when `centricmem` is on PATH.
- CLI **>=0.21.36**.

## 0.21.49

- After Skill install, the same reply must send https://centricmem.com/login?signup=1 unless `cm_health` already works. A working terminal is not the CentricMem CLI Ã¢â‚¬â€ `connect --device` only when `centricmem` is on PATH. Hermes: `~/.hermes/skills/` and Bearer in `~/.hermes/config.yaml`.
- CLI **>=0.21.36**.
...
- Ignore host `cm_doctor` saying the Skill is outdated Ã¢â‚¬â€ that is the librarian disk. Compare this file to `skill_latest` / GitHub.
- CLI **>=0.21.36**.
...
- `cm_move` `{from,to,files}` relocates selected Markdown cards onto another named shelf. Source files are deleted. Decision numbers stay if free on dest, otherwise a new seq. Attachments re-attach on dest. Default key or owner login; extra keys cannot. Pick cards with `cm_search` / `cm_show`. Do not download originals to Ã¢â‚¬Å“moveÃ¢â‚¬Â them. `cm_copy` still copies a whole leftover shelf; card delete stays login-only.

## 0.21.39

- Hosted ambient/doctor: unmatched cwd or omitted `shelf=` is `library=(none)` plus `libraries=` Ã¢â‚¬â€ not the hub `use` pin. Pick a named shelf this turn (or mint). Extra keys with exactly one grant still open that shelf. Guests do not `--link`.
...
- Kimi Code: `.kimi-plugin/plugin.json` plus a Custom catalog. Install with `/plugins install https://github.com/zeyu-j/centricmem-skill`. Host MCP Bearer is `CENTRICMEM_API_KEY` (never in chat). Official/Curated listing is a Moonshot PR Ã¢â‚¬â€ not a self-serve form.

## 0.21.37

- If the shell is blocked or `centricmem` is missing, do not retry CLI. Send https://centricmem.com/login?signup=1; the human adds `https://mem.centricmem.com/mcp` in this agentÃ¢â‚¬â„¢s MCP settings (key from Keys, never in chat). `connect --device` stays the path when CLI works.
...
- Public names: **Library** (one per person) Ã¢â€ â€™ **Shelf** (pairing-key vault) Ã¢â€ â€™ **Card**. Agents pass `shelf=<id>` or `library=<id>`. Default account key = every shelf in this library. Pairing key = that shelf. Inbox is a system shelf, never a sweep target.
...
- Authenticate is always a `/connect?device=` link. The agent runs `centricmem connect --device` and sends only that URL. You have **ten minutes** to enter the key. Sign in at the website for the dashboard Ã¢â‚¬â€ there is no Ã¢â‚¬Å“connect this computerÃ¢â‚¬Â button. Never paste a key into chat.
...
- Login uniquely owns delete and billing. The **default** account key (all libraries) may mint, rename, grant, and revoke other keys Ã¢â‚¬â€ including every pairing key on a library. Pairing keys cannot. New tokens stay off chat (dashboard / CLI, not MCP).
- Attachments are metered per plan (Lite 100MB, Education 200MB, Pro 1GB, Ultra 10GB). Markdown is unlimited. Over quota, keep fails Ã¢â‚¬â€ the agent says so.
...
- Not a one-click install. You enter the key on `https://centricmem.com/connect` Ã¢â‚¬â€ never in chat.

## 0.21.17

- When a key is needed, the agent sends the authenticate link `https://centricmem.com/connect`. You enter the key on that page Ã¢â‚¬â€ never in chat. Not a one-click install. The helper on this computer (`centricmem connect`) writes MCP for every agent.
...
- Sweep never writes Inbox (`unclassified`). Pick a named library, or mint one (`cm_library`) when none fits. Drain leftovers with classify Ã¢â‚¬â€ do not leave them for the human. Pairing keys still cannot mint a library.
...
- The agent talks to the librarian only through host MCP (`cm_health`, `cm_search`, `cm_keep`, Ã¢â‚¬Â¦). It does not curl. Originals stay on the dashboard.
...
- The agent files when the work is real Ã¢â‚¬â€ you do not have to say wrap up. Closing the tab does not file; it files before it stops.
...
- At close, the agent keeps this chatÃ¢â‚¬â„¢s transcript in object storage, then files units. You download originals from the dashboard.
