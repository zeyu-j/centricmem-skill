...
|---|---|---|
| **pi** 0.87.1 | `pi install https://github.com/zeyu-j/centricmem-skill` | clone lands in `~/.pi/agent/git/github.com/zeyu-j/centricmem-skill` and `pi list` reports it under user packages. Installed from `@earendil-works/pi-coding-agent` - the unscoped `pi` on npm is nothing to do with this |
| **Kiro CLI** | `kiro-cli mcp add --name centricmem --url https://mem.centricmem.com/mcp --scope default` | answers `✔ Added MCP server 'centricmem' to default config in ~/.kiro/settings/mcp.json`, and `kiro-cli mcp list` shows it under the default agent. Note the shape: this client takes flags only, so the name and url cannot be positional. Its `plugin`/marketplace manifests are for the Kiro IDE, not this CLI |
