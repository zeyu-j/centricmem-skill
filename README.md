...
| goose | `goose plugin install https://github.com/zeyu-j/centricmem-skill` - goose reads the manifest at the repository root; the recipes live in [`goose/`](./goose/) |
...
| Qwen Code | Copy [`qwen/qwen-extension.json`](./qwen/qwen-extension.json) and the hooks entry from `qwen/settings-hooks.example.json` - see [`qwen/README.md`](./qwen/README.md) |
...
| `.claude-plugin/`, `.codebuddy-plugin/`, `.agents/plugins/`, `.github/plugin/`, `.kiro/plugins/`, `.cursor-plugin/`, `.grok-plugin/`, `.kimi-plugin/` | per-client marketplaces and manifests |
