...
export interface HostLibraryBinding {
  id: string;
  token: string;
}
...
export interface HostServerOptions {
  home: string;
  /** Legacy/test: one token, bound to the Inbox library when a hub exists. */
  token?: string;
  libraries?: HostLibraryBinding[];
  port?: number;
  /** Bind address. Default 127.0.0.1. Use 0.0.0.0 only behind a proxy / until TLS. */
  bind?: string;
  /** If set, non-loopback callers must send X-CentricMem-Proxy. */
  proxySecret?: string;
  extraApiJson?: string[];
  persist?: boolean;
}
...
export interface HostServer {
  ...
  token: string;
  home: string;
  lastGuest: LastGuest | null;
  close: () => Promise<void>;
}

const VERB_ROUTES: Array<{ method: string; path: string; verb: HostVerb }> = [
...
function matchBinding(presented: string, bindings: HostLibraryBinding[]): HostLibraryBinding | undefined {
...
function sendDownload(res: http.ServerResponse, file: FileDownload): void {
...
function proxyHeader(req: http.IncomingMessage): string | undefined {
...
function headerValue(partHeaders: string, name: string): string | undefined {
  const re = new RegExp(`^${name}:\\s*(.+)$`, "im");
  return re.exec(partHeaders)?.[1]?.trim();
}

function parseMultipart(buf: Buffer, contentType: string): { fields: Record<string, string>; file?: KeepUpload } {
  const boundaryMatch = /boundary=(?:"([^"]+)"|([^;]+))/i.exec(contentType);
  if (!boundaryMatch) throw new HostApiError(400, "BAD_REQUEST", "multipart boundary missing.");
  const boundary = boundaryMatch[1] || boundaryMatch[2];
  const marker = Buffer.from(`--${boundary}`);
  const fields: Record<string, string> = {};
  let file: KeepUpload | undefined;
...
function routeFor(method: string, pathname: string): HostVerb | undefined {
  return VERB_ROUTES.find((r) => r.method === method && r.path === pathname)?.verb;
}

function projectOf(result: unknown): string | undefined {
...
async function readJsonBody(req: http.IncomingMessage, contentType: string): Promise<unknown> {
...
async function handleAccountRoute(
  ...
  persist: boolean,
  extraApiJson: string[] | undefined,
  sessionToken: string,
): Promise<unknown> {
  const pathname = url.pathname;
  if (method === "GET" && pathname === "/account") {
    const cat = catalogFile ? loadCatalog(catalogFile) : null;
    if (!cat) throw new HostApiError(503, "NO_CATALOG", "Librarian has no library catalog.");
    return {
      ok: true,
  ...
  if (method === "POST" && pathname === "/account/libraries") {
    ...
    if (!id) throw new HostApiError(400, "BAD_REQUEST", "Provide id.");
  ...
  if (method === "POST" && pathname === "/account/keys") {
    ...
    if (!library) throw new HostApiError(400, "BAD_REQUEST", "Provide library.");
    const minted = addLibraryKey(library, name || "unnamed", catalogFile);
    const cat = loadCatalog(catalogFile);
    if (persist && cat) writeGuestApiManifest(cat, { extraPaths: extraApiJson });
    return { ok: true, ...minted };
  }
  if (method === "POST" && pathname === "/account/keys/revoke") {
    const library = strField(body.library);
    const id = strField(body.id);
    if (!library || !id) throw new HostApiError(400, "BAD_REQUEST", "Provide library and id.");
...
export async function listenHostServer(opts: HostServerOptions): Promise<HostServer> {
  ...
  let lastGuest: LastGuest | null = null;
  ...
  let catalog: ReturnType<typeof ensureHubCatalog> | undefined;
  let bindings: HostLibraryBinding[];
  if (pinnedLibraries) {
  ...
  function liveBindings(): HostLibraryBinding[] {
    if (pinnedLibraries) return bindings;
    if (catalog?.file) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      let body: unknown;
      let keepUpload: KeepUpload | undefined;
...
function bindListen(server: http.Server, preferred: number, host: string): Promise<number> {
  return new Promise((resolve, reject) => {
    const tryPort = (port: number) => {
      ...
      const onListen = () => {
        ...
        else reject(new Error("Failed to bind listen port."));
      };
