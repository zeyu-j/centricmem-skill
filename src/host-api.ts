...
export interface HostVerbRequest {
  verb: HostVerb;
  home: string;
  /** Pairing key selected this library; writes cannot target another. */
  libraryId?: string;
  /** Owner session may pick any library; pairing keys stay pinned. */
  scope?: "pairing" | "owner";
...
const WRITE_VERBS = new Set<HostVerb>([
  "note",
  "log-decision",
  "done",
  "keep",
...
export function isWriteVerb(verb: HostVerb): boolean {
  return WRITE_VERBS.has(verb);
}

function q(query: HostVerbRequest["query"], key: string): string | undefined {
  const v = query[key];
  if (Array.isArray(v)) return v[0];
  return typeof v === "string" && v.trim() ? v : undefined;
}

function qAll(query: HostVerbRequest["query"], key: string): string[] {
  const v = query[key];
...
function flag(query: HostVerbRequest["query"], key: string): boolean {
  const v = q(query, key);
...
function parseFilters(pairs: string[]): Record<string, string> | undefined {
...
function assertHub(home: string): void {
  if (!isWorkspace(home) || !fs.existsSync(path.join(home, "projects"))) {
    throw new HostApiError(
      503,
      "HUB_INVALID",
      "This folder is not a CentricMem library. Open Manager and choose the folder that contains workspace.json and projects.",
    );
  }
  if (looksLikeClientFolder(home)) {
    throw new HostApiError(
      503,
      "HUB_INVALID",
      "The library path is the CentricMem client folder. Choose a memory library in Manager settings.",
    );
  }
}

function resolveWriteProject(home: string, req: HostVerbRequest, body: Record<string, unknown>): string {
  ...
  if (req.libraryId) {
    if (requested && requested !== req.libraryId) {
      throw new HostApiError(
        403,
        "LIBRARY_MISMATCH",
        `This pairing key is for library ${req.libraryId}.`,
      );
    }
  ...
  if (cwd) return matchProjectByCwd(home, cwd) ?? UNCLASSIFIED;
  return UNCLASSIFIED;
}

function resolveAmbientProject(home: string, req: HostVerbRequest, body: Record<string, unknown>): string | undefined {
  if (req.libraryId) return req.libraryId;
...
function pairingPinned(req: HostVerbRequest): boolean {
  return Boolean(req.libraryId) && req.scope !== "owner";
}

function resolveAttach(_home: string, _project: string, attach: string | undefined): string | undefined {
  if (!attach?.trim()) return undefined;
  const normalized = path.posix.normalize(attach.trim().replace(/\\/g, "/"));
  if (!normalized.startsWith("imported/") || normalized.split("/").includes("..")) {
    throw new HostApiError(
      400,
      "ATTACH_PATH_REJECTED",
      "Attach must already be under imported/, or upload bytes with keep. The librarian will not open a server path.",
    );
  }
  return normalized;
}

function indexAfterWrite(home: string, project?: string): void {
  try {
...
export class FileDownload {
  readonly download = true as const;
  constructor(
...
function resolveLibraryFile(home: string, project: string, relPath: string): { rel: string; abs: string } {
...
async function attachInfo(
  home: string,
  libraryId: string,
  attachRel: string,
): Promise<{ bytes?: number; store: "disk" | "r2" | "missing" }> {
  ...
  if (isR2Enabled()) {
    ...
    if (head.exists) return { bytes: head.size, store: "r2" };
  }
  return { store: "missing" };
}

function uniqueAttachRel(filename: string): string {
...
async function keepFromUpload(home: string, project: string, upload: KeepUpload, title?: string, tags?: string[]): Promise<unknown> {
  if (isR2Enabled()) {
...
export async function handleHostVerb(
  req: HostVerbRequest,
  extras?: { keepUpload?: KeepUpload; lastGuest?: unknown },
): Promise<unknown> {
  ...
  if (req.verb === "search") {
    ...
    const hasAddr = Boolean(
      tags.length || parsed.type || parsed.status || parsed.agent || parsed.andTokens.length || parsed.projectScopes.length
    ...
    const filters = {
      ...
      meta: parseFilters(qAll(req.query, "filter").concat(
        typeof body.filter === "string" ? [body.filter] : Array.isArray(body.filter) ? body.filter.map(String) : [],
      )),
  ...
  if (req.verb === "show") {
    ...
    if (!file) throw new HostApiError(400, "BAD_REQUEST", "Provide file.");
    ...
    try {
      ...
      if (!original) return { ok: true, file, original: false, text };
      const attachRel = parseAttachLine(text);
      if (!attachRel) throw new HostApiError(400, "BAD_REQUEST", `No Attach line on ${file}`);
  ...
  if (req.verb === "classify") {
    ...
    if (!relPath || !to) throw new HostApiError(400, "BAD_REQUEST", "Provide relPath and to.");
    if (pairingPinned(req) && req.libraryId !== UNCLASSIFIED) {
  ...
  if (req.verb === "index") {
    ...
    const stats = wrapFs(() =>
      !pairingPinned(req) && all ? buildIndexAll(home, { quiet: true }) : buildIndex(resolvePaths(home, project)),
    );
  ...
  if (req.verb === "note") {
    ...
    if (!title || !noteBody) throw new HostApiError(400, "BAD_REQUEST", "Provide title and body.");
    ...
    if (r.status !== "skipped") indexAfterWrite(home, project);
    return { ok: true, status: r.status, title, project };
  }

  if (req.verb === "log-decision") {
    const title = str(body.title);
    if (!title) throw new HostApiError(400, "BAD_REQUEST", "Provide title.");
    const project = resolveWriteProject(home, req, body);
    const tags = parseTags(body.tags);
    const attach = resolveAttach(home, project, str(body.attach));
  ...
  if (req.verb === "done") {
    ...
    if (!summary) throw new HostApiError(400, "BAD_REQUEST", "Provide summary.");
    const project = resolveWriteProject(home, req, body);
    const tags = parseTags(body.tags);
    const attach = resolveAttach(home, project, str(body.attach));
  ...
  if (req.verb === "keep-sign") {
    ...
    if (!cfg) {
      throw new HostApiError(
        503,
        "R2_NOT_CONFIGURED",
        "Object store is not configured. Small files: POST /keep with filename+content. Large files need R2.",
      );
    }
    const project = resolveWriteProject(home, req, body);
  ...
  if (req.verb === "keep") {
    ...
    if (filename && contentBase64) {
      ...
      if (!bytes.length) throw new HostApiError(400, "BAD_REQUEST", "Empty keep payload.");
      return await keepFromUpload(home, project, { filename, bytes }, title, tags);
    }
    const content = str(body.content);
    if (filename && content != null) {
      return await keepFromUpload(
        home,
        project,
        { filename, bytes: Buffer.from(content, "utf8") },
        title,
        tags,
      );
    }
    ...
    if (src) {
      throw new HostApiError(
        400,
        "KEEP_PATH_REJECTED",
        "Keep only accepts uploaded bytes (filename + content) or a presigned uploadId. The librarian will not open a server path.",
      );
    }
  ...
  if (req.verb === "import") {
    ...
    if (raw == null) throw new HostApiError(400, "BAD_REQUEST", "Provide an ImportBundle JSON body.");
    const bundle = parseImportBundle(typeof raw === "string" ? raw : raw as object);
  ...
  if (req.verb === "delete") {
    ...
    let attachRel: string | undefined;
    try {
