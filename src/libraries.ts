...
export function activeKeys(lib: LibraryRecord): LibraryKey[] {
  return (lib.keys ?? []).filter((k) => k.token && !k.revokedAt);
}

function firstActiveToken(lib: LibraryRecord): string {
  return activeKeys(lib)[0]?.token || "";
}

function parseKeys(row: { token?: unknown; keys?: unknown }): LibraryKey[] {
  const keys: LibraryKey[] = [];
  if (Array.isArray(row.keys)) {
    for (const item of row.keys) {
      if (!item || typeof item !== "object") continue;
      const rec = item as Partial<LibraryKey>;
      if (typeof rec.id !== "string" || !rec.id.trim()) continue;
      if (typeof rec.token !== "string" || !rec.token.trim()) continue;
      keys.push({
        id: rec.id.trim(),
...
function ensureKeys(lib: LibraryRecord): LibraryRecord {
  if (!lib.keys?.length && lib.token) {
    lib.keys = [{ id: newKeyId(), name: "default", token: lib.token, createdAt: nowIso() }];
  }
  if (!lib.token) lib.token = firstActiveToken(lib);
  return lib;
}

function userApiPath(): string {
...
export interface LibraryRecord {
  ...
  memDir: string;
  sourceDirs: string[];
  /** First active pairing key (guest api.json / rotate). Empty if all keys are revoked. */
  token: string;
  keys: LibraryKey[];
  displayName: string;
  system?: boolean;
...
export interface LibraryCatalog {
  ...
  /** Guest librarian origin (`https://mem.example`). Env `CENTRICMEM_URL` wins when both are set. */
  origin?: string;
  /** Runtime only — path this catalog was loaded/saved from. Not written to disk. */
  file?: string;
}
...
export function librariesJsonPath(): string {
...
export function libraryDisplayName(id: string, configured?: string): string {
  if (configured?.trim()) return configured.trim();
  if (id === UNCLASSIFIED) return "Inbox";
  if (id === "host") return "This PC";
  return id;
}

function readConfigDisplayName(memDir: string): string | undefined {
  try {
    const raw = JSON.parse(fs.readFileSync(path.join(memDir, "config.json"), "utf8")) as {
...
export function loadCatalog(file = librariesJsonPath()): LibraryCatalog | null {
  try {
    const raw = JSON.parse(fs.readFileSync(file, "utf8")) as Partial<LibraryCatalog>;
    if (raw.version !== CATALOG_VERSION || typeof raw.hub !== "string") return null;
    ...
    for (const row of raw.libraries) {
      ...
      const token =
        ...
        "";
      const memDir =
        typeof row.memDir === "string" && row.memDir.trim()
          ? path.resolve(row.memDir)
          : projectMemDir(hub, id);
      const sourceDirs = Array.isArray(row.sourceDirs)
        ? row.sourceDirs.filter((s): s is string => typeof s === "string" && Boolean(s.trim())).map((s) => path.resolve(s))
        : [];
    ...
    const current =
      typeof raw.current === "string" && libraries.some((l) => l.id === raw.current)
        ? raw.current
        : libraries[0].id;
...
export function saveCatalog(catalog: LibraryCatalog, file = librariesJsonPath()): void {
...
function recordFromWorkspace(
  hub: string,
  id: string,
  entry: WorkspaceConfig["projects"][string],
...
export function ensureHubCatalog(
  hub: string,
  opts?: { bindToken?: string; file?: string; port?: number },
): LibraryCatalog {
  ...
  if (machineDefault && existing && path.resolve(existing.hub) !== resolved) {
    file = path.join(
      os.tmpdir(),
      "centricmem-catalogs",
      crypto.createHash("sha1").update(resolved.toLowerCase()).digest("hex"),
      "libraries.json",
    );
  ...
  if (opts?.bindToken?.trim()) {
    ...
    if (inbox) {
      ...
      else keys.push({ id: newKeyId(), name: "default", token, createdAt: nowIso() });
      inbox.keys = keys;
    }
  }
  const current =
    existing?.current && libraries.some((l) => l.id === existing.current)
      ? existing.current
...
export function findLibraryByToken(
  token: string,
  catalog?: LibraryCatalog | null,
): LibraryRecord | undefined {
  const cat = catalog ?? loadCatalog();
  if (!cat || !token) return undefined;
...
export function matchLibraryByCwd(
  cwd: string,
  catalog?: LibraryCatalog | null,
): LibraryRecord | undefined {
  const cat = catalog ?? loadCatalog();
  ...
  let best: { lib: LibraryRecord; len: number } | undefined;
  for (const lib of cat.libraries) {
    for (const dir of lib.sourceDirs) {
      ...
      if (needle === root || needle.startsWith(root + "/")) {
        if (!best || root.length > best.len) best = { lib, len: root.length };
      }
    }
  }
  return best?.lib;
...
export function rotateLibraryToken(id: string, file?: string): LibraryRecord {
  const pathFile = file ?? librariesJsonPath();
  const cat = loadCatalog(pathFile);
  if (!cat) throw new Error("No library catalog. Open Manager or run centricmem libraries.");
  const lib = cat.libraries.find((row) => row.id === id);
  if (!lib) throw new Error(`Unknown library: ${id}`);
  const now = nowIso();
  for (const key of lib.keys ?? []) {
    if (!key.revokedAt && key.token === lib.token) key.revokedAt = now;
  }
  const token = generateToken();
  const key: LibraryKey = { id: newKeyId(), name: "default", token, createdAt: now };
...
export function addLibraryKey(
  id: string,
  name: string,
  file?: string,
): { id: string; name: string; token: string; createdAt: string; library: string } {
  const pathFile = file ?? librariesJsonPath();
  const cat = loadCatalog(pathFile);
  if (!cat) throw new Error("No library catalog. Open Manager or run centricmem libraries.");
  const lib = cat.libraries.find((row) => row.id === id);
  ...
  lib.keys = [...(lib.keys ?? []), key];
  if (!lib.token) lib.token = token;
  saveCatalog(cat, pathFile);
  return { id: key.id, name: key.name, token, createdAt: key.createdAt, library: lib.id };
}

export function revokeLibraryKey(libraryId: string, keyId: string, file?: string): LibraryRecord {
  const pathFile = file ?? librariesJsonPath();
  const cat = loadCatalog(pathFile);
  if (!cat) throw new Error("No library catalog.");
  ...
  if (!key.revokedAt) key.revokedAt = nowIso();
  if (lib.token === key.token) lib.token = firstActiveToken(lib);
  saveCatalog(cat, pathFile);
  return lib;
}

export function listLibraryKeys(libraryId: string, file?: string): Array<{
  id: string;
  name: string;
  createdAt: string;
  revokedAt?: string;
...
export function setCurrentLibrary(id: string, file?: string): LibraryCatalog {
  const pathFile = file ?? librariesJsonPath();
  const cat = loadCatalog(pathFile);
  if (!cat) throw new Error("No library catalog.");
  if (!cat.libraries.some((lib) => lib.id === id)) throw new Error(`Unknown library: ${id}`);
  cat.current = id;
  saveCatalog(cat, pathFile);
  try {
...
export function createLibrary(
  hub: string,
  id: string,
  opts?: { sourceDir?: string; displayName?: string; file?: string },
): LibraryRecord {
  if (!/^[a-z0-9][a-z0-9_-]*$/i.test(id)) throw new Error(`Invalid library id: ${id}`);
  const resolved = path.resolve(hub);
  ...
  const lib = cat.libraries.find((row) => row.id === id);
  if (!lib) throw new Error(`Failed to register library: ${id}`);
  if (opts?.displayName) {
...
export function linkCwdToLibrary(
  hub: string,
  codePath: string,
  ...
  const lib = cat.libraries.find((row) => row.id === id);
  if (!lib) throw new Error(`Failed to link library: ${id}`);
...
export function writeGuestApiManifest(
  ...
  for (const extra of opts?.extraPaths ?? []) files.push(extra);
  for (const file of [...new Set(files.map((p) => path.resolve(p)))]) {
    atomicJson(file, body);
  }
}

export function tokenBindings(catalog: LibraryCatalog): Array<{ id: string; token: string }> {
...
export function publicLibraryKeys(lib: LibraryRecord): Array<{
  id: string;
  name: string;
  createdAt: string;
  revokedAt?: string;
...
export function stripHubApiToken(hub: string, port = DEFAULT_PORT): void {
...
export function formatLibrariesList(catalog: LibraryCatalog): string {
