...
export function objectKeyFor(libraryId: string, attachRel: string): string {
...
function iso8601(d = new Date()): { amzDate: string; dateStamp: string } {
...
export function presignPut(cfg: R2Config, objectKey: string, ttlSec = R2_SIGN_TTL_SEC): PresignPut {
...
export async function r2Put(objectKey: string, bytes: Buffer): Promise<void> {
...
export interface FillAttachResult {
  libraryId: string;
  scanned: number;
  uploaded: number;
  skipped: number;
...
function listAttachFiles(dir: string): { abs: string; attachRel: string; size: number }[] {
  const root = path.resolve(dir);
  if (!fs.existsSync(root) || !fs.statSync(root).isDirectory()) {
    throw new Error(`Not a directory: ${root}`);
  }
  const out: { abs: string; attachRel: string; size: number }[] = [];
...
export async function fillAttachFromDir(opts: {
  ...
  concurrency?: number;
}): Promise<FillAttachResult> {
  const libraryId = opts.libraryId.trim();
  if (!libraryId) throw new Error("Provide --library.");
  if (!opts.dryRun && !isR2Enabled()) throw new Error("R2 is not configured.");
  if (opts.deleteSource && opts.dryRun) throw new Error("--delete-source cannot be used with --dry-run.");
  const files = listAttachFiles(opts.dir);
  const result: FillAttachResult = {
    libraryId,
...
export async function r2Head(objectKey: string): Promise<{ exists: boolean; size?: number }> {
  const cfg = readR2Config();
...
export async function r2Get(objectKey: string): Promise<Buffer> {
  const cfg = readR2Config();
  if (!cfg) throw new Error("R2 is not configured.");
  const res = await signedFetch(cfg, "GET", objectKey);
  if (res.status === 404) throw new Error(`Attached original missing: ${objectKey}`);
...
export async function loadAttachOriginal(home: string, libraryId: string, attachRel: string): Promise<Buffer> {
  const rel = attachRel.replace(/\\/g, "/");
  const paths = resolvePaths(home, libraryId);
  const abs = path.join(paths.memDir, ...rel.split("/"));
  if (fs.existsSync(abs) && fs.statSync(abs).isFile()) return fs.readFileSync(abs);
  if (isR2Enabled()) return r2Get(objectKeyFor(libraryId, rel));
...
const pending = new Map<string, PendingKeep>();

export function createPendingKeep(row: Omit<PendingKeep, "expiresAt">): { uploadId: string; expiresAt: string } {
...
export function takePendingKeep(uploadId: string): PendingKeep {
  const row = pending.get(uploadId);
  if (!row) throw new Error("Unknown or already used keep upload.");
...
export function peekPendingKeep(uploadId: string): PendingKeep | undefined {
