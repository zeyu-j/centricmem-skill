...
export interface SetupOptions {
  ...
  /** Remember library path (pointer file + Windows user env). */
  persistHome?: boolean;
  /** Copy hub files from fromHome (or current library) into --workspace. */
  migrateHome?: boolean;
  /** Source hub for migrateHome (default: current getProductHome()). */
  fromHome?: string;
  /** After migrateHome, rename workspace.json if source was the CLI folder. */
...
export interface SetupResult {
  ...
  migratedHome: boolean;
  persistedHome: string | null;
  retiredOldHome: boolean;
...
export function runSetup(opts: SetupOptions = {}): SetupResult {
  ...
  let persistedHome: string | null = null;
  if (opts.persistHome) {
...
function copyEntryPreservingLinks(src: string, dest: string): void {
  ...
  if (st.isSymbolicLink()) {
    ...
    if (fs.existsSync(dest) || fs.lstatSync(dest, { throwIfNoEntry: false })) {
      try {
        ...
        if (dst.isSymbolicLink()) fs.unlinkSync(dest);
 ...
 */
export function migrateProductHome(fromHome: string, toHome: string): boolean {
...
export function retireMixedHub(dir: string): boolean {
  const root = path.resolve(dir);
  if (!looksLikeClientFolder(root)) return false;
...
export function retargetJunction(linkPath: string, newTarget: string): void {
...
function installSkillToHome(productHome: string, guest = false): boolean {
  const here = path.dirname(fileURLToPath(import.meta.url));
  const skillDir = path.resolve(here, "../skills/centricmem-agent");
  if (!fs.existsSync(path.join(skillDir, "SKILL.md"))) return false;
