...
async function call(pathname, init = {}, library) {
    const manifest = resolveGuestManifest({ library });
    if (!manifest)
        throw new Error(downMessage());
...
server.registerTool("cm_health", { title: "Librarian health", description: "Check that the CentricMem librarian HTTP API is reachable.", inputSchema: {} }, async () => wrap(() => call("/health")));
server.registerTool("cm_ambient", {
...
server.registerTool("cm_log_decision", {
...
server.registerTool("cm_keep", {
    ...
    if (!signRes.ok) {
        try {
            ...
            if (error instanceof Error && error.message !== signedText)
                throw error;
    ...
    if (!put.ok)
        throw new Error(`Object store PUT failed (${put.status}).`);
    return call("/keep", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
