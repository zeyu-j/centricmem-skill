...
import { cliVersion, skillStatus, packageRoot } from "./skill.js";
import { getCurrentProjectSlug, isWorkspace, loadWorkspace, matchProjectByCwd } from "./workspace.js";
import { probeLibrarian } from "./host-discover.js";
import { ensureHubCatalog, loadCatalog, matchLibraryByCwd } from "./libraries.js";
import { isLibrarianGuest, librarianGuestOrigin } from "./guest.js";
export async function runDoctor(workspaceRoot, cwd = process.cwd(), opts) {
    ...
    if (looksLikeClientFolder(home)) {
        errors.push(`Library is the CentricMem client folder (${home}). Choose a memory library: centricmem setup --workspace <path> --persist-home --migrate-home`);
    }
    const guest = isLibrarianGuest();
    const guestOrigin = librarianGuestOrigin();
    if (guest) {
        warnings.push(`Guest of ${guestOrigin} — leftover CENTRICMEM_HOME is ignored for writes. Do not start a local Manager listen.`);
    }
    if (resolved.source === "env-client-ignored" && !workspaceRoot?.trim()) {
        warnings.push(`This shell still has CENTRICMEM_HOME pointing at the client; using the library pointer. Restart the terminal/Cursor so the user env matches.`);
    }
    if (resolved.source === "default" && !workspaceRoot?.trim()) {
        warnings.push(`Library is the default ~/.centricmem. To put memory on another drive: centricmem setup --workspace <path> --persist-home`);
    }
    if (!hub_writable) {
    ...
    let skill_status = "uninitialized";
    let project = null;
    let library = null;
    let libraries = [];
    let cwd_match = null;
    let workspace_current = null;
    if (!initialized && !guest) {
        ...
        if (!cwd_match) {
            warnings.push(`cwd not linked in the guest catalog (${cwd}) — HTTP uses Inbox or CENTRICMEM_PROJECT.`);
        }
    }
    else {
        const skill = skillStatus(home);
        ...
        try {
            ...
            if (!cwd_match) {
                warnings.push(`cwd not linked (${cwd}) — writes go to the Inbox library; workspace.current=${workspace_current}; centricmem setup --link <cwd> or CENTRICMEM_PROJECT=<library>`);
            }
        }
        catch (e) {
    ...
    const librarian = opts?.skipLibrarianProbe
        ? { listening: true, auth: "ok", host: "127.0.0.1" }
...
function formatLibrarian(r) {
    const lib = r.librarian;
    const origin = (process.env.CENTRICMEM_URL?.trim() || loadCatalog()?.origin || "").replace(/\/+$/, "");
    const where = origin && /^https?:\/\//i.test(origin)
        ? origin
        : lib.host && lib.host !== "127.0.0.1" && lib.host !== "localhost"
            ? `${lib.host}:${lib.port ?? "?"}`
            : `:${lib.port ?? "?"}`;
    if (!lib.listening)
        return `Librarian: not reachable (${where})`;
    if (lib.auth === "ok")
        return `Librarian: ${where} (token ok)`;
    if (lib.auth === "unauthorized")
        return `Librarian: ${where} (token failed)`;
    if (lib.auth === "missing_token")
        return `Librarian: ${where} (no token)`;
    return `Librarian: ${lib.auth}`;
}
export function formatDoctorText(r) {
    const lines = [
        `CLI:     ${r.cli_version}`,
        `Client:  ${r.client}`,
