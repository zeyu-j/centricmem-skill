...
export function legacyInstalledSkillPath(workspaceRoot, name) {
...
export function readSkillInfo(absPath) {
...
export function satisfiesCliRange(cliVer, range) {
...
export function skillStatus(workspaceRoot, opts) {
    ...
    if (compatible && !satisfiesCliRange(cli, compatible)) {
        ...
        status = "missing";
        const legacyExists = !opts?.installPath && readSkillInfo(legacyInstalledSkillPath(workspaceRoot, name));
        hint = legacyExists
...
export function formatSkillStatusText(r) {
    const lines = [
        `  skill:     ${r.name}`,
        `  cli:       ${r.cli_version}`,
    ...
    if (r.hint)
        lines.push("", `  → ${r.hint}`);
    if (r.changelog_url)
        lines.push(`  → ${r.changelog_url}`);
    return lines.join("\n");
}
/** Hub-level cold start — distinct from skill status `missing` (hub exists, Skill file absent). */
export function formatUninitializedSkillStatus(home, name = "centricmem-agent") {
    return {
        hub: "UNINITIALIZED",
        home,
        name,
...
export function formatUninitializedSkillStatusText(home, name = "centricmem-agent") {
...
export function skillStatusHintLine(r) {
    if (r.status === "ok")
        return null;
