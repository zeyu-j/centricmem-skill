...
`;
export function sweepDbPath(indexDir) {
    return path.join(indexDir, "sweep.db");
}
export function openSweepDb(file) {
...
function occupiedCount(db, libraryId, now, windowMs) {
    const since = now - windowMs;
    const rows = db
        .prepare(`SELECT created_at AS createdAt, committed_at AS committedAt, expires_at AS expiresAt, created_units AS createdUnits
...
export function remainingSweeps(db, libraryId, opts) {
    ...
    const retryAfterMs = remaining > 0 || oldestOccupying === null ? 0 : Math.max(0, oldestOccupying + windowMs - now);
    return { remaining, retryAfterMs };
}
/** Take one slot for an in-flight sweep. Concurrent callers serialize on sqlite. */
export function reserveSweep(db, libraryId, opts) {
    const limit = opts?.limit ?? DEFAULT_SWEEPS_PER_WINDOW;
    const now = opts?.now ?? Date.now();
    const windowMs = opts?.windowMs ?? SWEEP_WINDOW_MS;
    ...
    const run = db.transaction(() => {
        const { n, oldestOccupying } = occupiedCount(db, libraryId, now, windowMs);
        if (n >= limit) {
            const retryAfterMs = oldestOccupying === null ? windowMs : Math.max(0, oldestOccupying + windowMs - now);
            return { ok: false, code: "SWEEP_LIMIT", remaining: 0, retryAfterMs };
        }
        const id = crypto.randomBytes(16).toString("hex");
        db.prepare(`INSERT INTO sweep_reservations(id, library_id, created_at, expires_at, committed_at, created_units)
       VALUES (?, ?, ?, ?, NULL, 0)`).run(id, libraryId, now, now + holdMs);
        return { ok: true, reservationId: id, remainingAfterReserve: Math.max(0, limit - n - 1) };
    });
    return run();
}
export function abortSweep(db, reservationId) {
 ...
 */
export function commitSweep(db, reservationId, externalIds, opts) {
    const now = opts?.now ?? Date.now();
    const run = db.transaction(() => {
        const row = db
            ...
            .get(reservationId);
        if (!row)
            throw new Error("Unknown sweep reservation.");
        if (row.committedAt != null) {
            const createdUnits = db.prepare(`SELECT created_units AS n FROM sweep_reservations WHERE id = ?`).get(reservationId).n;
            return { createdUnits, counted: createdUnits > 0 };
        }
        let created = 0;
        const ins = db.prepare(`INSERT INTO sweep_idempotency(library_id, external_id, reservation_id) VALUES (?, ?, ?)
       ON CONFLICT(library_id, external_id) DO NOTHING`);
        ...
        db.prepare(`UPDATE sweep_reservations SET committed_at = ?, created_units = ?, expires_at = ? WHERE id = ?`).run(now, created, now, reservationId);
