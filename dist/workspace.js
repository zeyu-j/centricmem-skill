...
import { ensureDir, nowISO, slugify, LOCAL_MEM_DIR, getProductHome, projectMemDir, looksLikeClientFolder, } from "./core.js";
...
export function getWorkspaceCurrent(workspaceRoot) {
...
export function classifyMemory(workspaceRoot, relPath, toSlug) {
    ...
    if (src.endsWith(".md")) {
        ...
        if (attachRel) {
            ...
            if (attachSrc.startsWith(fromDir + path.sep) &&
...
function companionAttachRel(content) {
...
function projectClassifyHints(workspaceRoot, slug) {
    const file = path.join(projectMemDir(workspaceRoot, slug), "config.json");
    if (!fs.existsSync(file))
        return [];
    try {
        const raw = JSON.parse(fs.readFileSync(file, "utf8"));
        if (!Array.isArray(raw.classify_hints))
            return [];
...
function listInboxFileRels(unclassifiedDir) {
    const out = [];
        ...
        const walk = (dir, rel) => {
            for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
                ...
                const isDir = e.isDirectory() || (e.isSymbolicLink() && (() => {
                    try {
                        return fs.statSync(childAbs).isDirectory();
                    }
                    catch {
                        return false;
                    }
...
export function highConfidenceTarget(suggestions) {
    const top = suggestions[0];
    if (!top || top.score < INBOX_APPLY_MIN_SCORE)
        return null;
    const second = suggestions[1];
    if (second && top.score - second.score < INBOX_APPLY_LEAD)
        return null;
    return top;
}
/** Independent files + whole-file aggregates under unclassified. */
export function listInbox(workspaceRoot) {
...
export function countInboxFiles(workspaceRoot) {
...
export function applyInbox(workspaceRoot, minScore = INBOX_APPLY_MIN_SCORE) {
    const moved = [];
    const skipped = [];
