...
export function userApiJsonPath() {
...
export function hubApiJsonPath(home) {
    return path.join(home, "api.json");
}
export function apiJsonCandidates(home) {
    ...
    const out = [];
    if (resolvedHome)
        out.push(hubApiJsonPath(resolvedHome));
    ...
    if (!out.includes(besidePointer))
        out.push(besidePointer);
    return [...new Set(out.map((p) => path.resolve(p)))];
}
function manifestFromLibrary(cat, lib) {
    return {
        host: "127.0.0.1",
        port: cat.port,
        token: lib.token,
...
function catalogLibrarianOrigin() {
    try {
...
function withEnvLibrarian(manifest) {
...
export function resolveGuestManifest(opts) {
...
export function readApiManifest(home) {
    ...
    for (const file of apiJsonCandidates(home)) {
        ...
        if (parsed)
            return withEnvLibrarian(parsed);
    }
    return envOnlyManifest();
}
export function readApiManifestFile(file) {
    try {
        ...
        return {
            ...
            libraries: Array.isArray(raw.libraries)
                ? raw.libraries
                    .filter((row) => Boolean(row && typeof row === "object" && typeof row.id === "string" && typeof row.token === "string"))
                    .map((row) => ({
                    id: String(row.id),
                    displayName: typeof row.displayName === "string"
                        ? row.displayName
                        : String(row.id),
...
export function writeApiManifest(manifest, extraPaths = []) {
...
export function librarianBaseUrl(manifest) {
    const origin = envLibrarianOrigin();
    if (origin) {
        const hidePort = (origin.protocol === "http" && origin.port === 80) ||
...
export function joinLibrarianUrl(base, verbPath) {
    const origin = base.replace(/\/+$/, "");
    if (!verbPath)
        return origin;
    if (/^https?:\/\//i.test(verbPath))
        return verbPath;
    const path = verbPath.startsWith("/") ? verbPath : `/${verbPath}`;
    return `${origin}${path}`;
}
export function healthUrl(manifest) {
    return joinLibrarianUrl(librarianBaseUrl(manifest), "/health");
}
function probeTarget(manifest) {
    const origin = envLibrarianOrigin();
    if (origin) {
        return {
            host: origin.host,
...
export async function probeLibrarian(manifest) {
...
export async function librarianRequest(verbPath, init = {}) {
...
export function healthPayload(home) {
    return {
        ok: true,
        protocol: HOST_PROTOCOL,
