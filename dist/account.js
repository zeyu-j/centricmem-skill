...
export function ownerPath(home) {
...
function scryptHash(password, salt, n = SCRYPT_N, r = SCRYPT_R, p = SCRYPT_P, keylen = KEYLEN) {
...
export function loadOwner(home) {
    try {
        const raw = JSON.parse(fs.readFileSync(ownerPath(home), "utf8"));
        if (raw.version !== 1 || typeof raw.email !== "string")
            return null;
...
export function hasOwner(home) {
    return loadOwner(home) !== null;
}
export function bootstrapOwner(home, email, password) {
    const resolved = path.resolve(home);
    if (loadOwner(resolved)) {
        throw new AccountError("OWNER_EXISTS", "Owner already exists. There is no public register. Sign in instead.", 409);
    }
...
export function registerOwner(home, email, password) {
    bootstrapOwner(home, email, password);
    return loginOwner(home, email, password);
}
function passwordOk(owner, password) {
    const salt = Buffer.from(owner.password.salt, "base64");
    const expected = Buffer.from(owner.password.hash, "base64");
    const got = scryptHash(password, salt, owner.password.n, owner.password.r, owner.password.p, owner.password.keylen);
    return equalBuf(got, expected);
}
function pruneSessions(owner, now = Date.now()) {
...
export function loginOwner(home, email, password) {
...
export async function requestOwnerPasswordReset(home, email) {
    if (!mailConfigured()) {
...
export function resetOwnerPassword(home, token, password) {
...
export function verifyOwnerSession(home, token) {
...
export function logoutOwnerSession(home, token) {
    const parsed = parseSessionToken(token);
    if (!parsed)
        return false;
    const owner = loadOwner(home);
    ...
    if (owner.sessions.length === before)
        return false;
    saveOwner(home, owner);
    return true;
}
...
export function loadGuestSession() {
    try {
        const raw = JSON.parse(fs.readFileSync(guestSessionPath(), "utf8"));
...
export async function accountFetch(url, token, method, pathname, body) {
