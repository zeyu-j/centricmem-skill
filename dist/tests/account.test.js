...
after(() => {
    if (prevDump === undefined)
        ...
        process.env.CENTRICMEM_MAIL_DUMP = prevDump;
    if (prevResend === undefined)
        delete process.env.CENTRICMEM_RESEND_API_KEY;
    else
        process.env.CENTRICMEM_RESEND_API_KEY = prevResend;
    if (prevOrigin === undefined)
        delete process.env.CENTRICMEM_APP_ORIGIN;
    else
        process.env.CENTRICMEM_APP_ORIGIN = prevOrigin;
...
describe("account", { concurrency: 1 }, () => {
    test("bootstrap refuses a second owner", () => {
        ...
        assert.throws(() => bootstrapOwner(home, "other@example.com", "other-secret-1"), (err) => err instanceof AccountError && err.code === "OWNER_EXISTS");
    });
    test("POST /register creates the owner; a second register is 409", async () => {
        ...
        try {
            ...
            assert.equal((await open.json()).registered, false);
    ...
    test("forgot password emails a link; reset replaces the password and sessions", async () => {
        ...
        try {
            ...
            const mail = JSON.parse(fs.readFileSync(dump, "utf8"));
            ...
            const session = await reset.json();
            assert.equal(session.email, EMAIL);
            assert.match(session.token, /^cm_sess_/);
