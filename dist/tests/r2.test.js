...
test("presign PUT then keep uploadId stores bytes in R2 not on the hub disk", async () => {
    ...
    try {
        ...
        const orig = await fetch(`http://127.0.0.1:${server.port}/download?file=${encodeURIComponent(logged)}&original=1`, { headers: { Authorization: `Bearer ${token}` } });
