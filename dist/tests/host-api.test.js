...
test("POST /keep with bytes lands in imported/attach", async () => {
    ...
    await withServer(home, async (port, token) => {
        ...
        const shown = await fetch(`http://127.0.0.1:${port}/show?file=${encodeURIComponent(body.stub)}&original=1`, { headers: { Authorization: `Bearer ${token}` } });
        assert.equal(shown.status, 200);
        const ptr = await shown.json();
        ...
        const listed = await fetch(`http://127.0.0.1:${port}/search?q=${encodeURIComponent("Kept memo")}&project=unclassified`, { headers: { Authorization: `Bearer ${token}` } });
        assert.equal(listed.status, 200);
        const hits = await listed.json();
        const stubHit = (hits.results || []).find((row) => row.file === body.stub);
        assert.equal(stubHit?.attach, body.attach);
        const dl = await fetch(`http://127.0.0.1:${port}/download?file=${encodeURIComponent(body.stub)}&original=1`, { headers: { Authorization: `Bearer ${token}` } });
...
test("search without a query is a HostApiError", async () => {
    ...
    await assert.rejects(() => handleHostVerb({ verb: "search", home, query: {} }), (err) => err instanceof HostApiError && Number(err.http) === 400);
});
test("a library pairing key cannot write into another library", async () => {
    ...
    const demo = cat.libraries.find((l) => l.id === "demo");
    const inbox = cat.libraries.find((l) => l.id === "unclassified");
...
test("handleHostVerb delete removes the file", async () => {
    ...
    await assert.rejects(() => handleHostVerb({
        verb: "download",
        home,
        libraryId: "unclassified",
        query: { file: rel },
    }), (err) => err instanceof HostApiError && Number(err.http) === 404);
});
...
test("note attach cannot point at a server path or escape imported/", async () => {
    ...
    }), (err) => err instanceof HostApiError && err.code === "ATTACH_PATH_REJECTED");
    await assert.rejects(() => handleHostVerb({
        verb: "note",
        home,
        libraryId: "unclassified",
...
test("CORS does not reflect an unknown Origin", async () => {
    ...
    try {
        ...
        if (prev === undefined)
            delete process.env.CENTRICMEM_CORS_ORIGIN;
        else
            process.env.CENTRICMEM_CORS_ORIGIN = prev;
    }
});
test("CENTRICMEM_URL with a path prefix keeps /api on verbs", async () => {
    ...
    try {
        ...
        if (prev === undefined)
            ...
            process.env.CENTRICMEM_URL = prev;
    }
});
