...
const {
  ...
  rotateLibraryToken,
  saveCatalog,
} = await import(toImport(path.join(distDir, "libraries.js")));
...
after(() => {
  if (prevCatalog === undefined) delete process.env.CENTRICMEM_LIBRARIES_JSON;
  else process.env.CENTRICMEM_LIBRARIES_JSON = prevCatalog;
...
test("ensureHubCatalog wraps workspace slugs and preserves tokens", () => {
  ...
  assert.ok(first.libraries.some((l: { id: string }) => l.id === "unclassified"));
  assert.ok(first.libraries.some((l: { id: string }) => l.id === "demo"));
  const inbox = first.libraries.find((l: { id: string }) => l.id === "unclassified");
  const token = inbox.token;
  const second = ensureHubCatalog(home);
  assert.equal(second.libraries.find((l: { id: string }) => l.id === "unclassified").token, token);
...
test("rotateLibraryToken changes only that library", () => {
  ...
  const a0 = cat.libraries.find((l: { id: string }) => l.id === "a").token;
  const b0 = cat.libraries.find((l: { id: string }) => l.id === "b").token;
  const rotated = rotateLibraryToken("a");
  assert.notEqual(rotated.token, a0);
  assert.equal(loadCatalog()!.libraries.find((l: { id: string }) => l.id === "b").token, b0);
});
...
test("a library can hold a second named key; revoke leaves the other", () => {
  ...
  ensureProjectRegistered(home, "demo");
  const cat = ensureHubCatalog(home);
  const primary = cat.libraries.find((l: { id: string }) => l.id === "demo");
  assert.ok(primary);
  ...
  assert.equal(findLibraryByToken(primary.token)?.id, "demo");
  assert.equal(loadCatalog()!.libraries.find((l: { id: string }) => l.id === "demo").token, primary.token);
});
