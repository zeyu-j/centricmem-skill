...
const { initProject, logDecision, updateContext, readContext, healthCheck, autoSessionSummary, logSession, logLesson, countTodaySessions, keepOriginal, showMemory } =
...
const { linkProject, useProject, listProjects, classifyMemory, UNCLASSIFIED, workspaceHealth, loadWorkspace, saveWorkspace, getCurrentProjectSlug, listInbox, applyInbox, ensureProjectRegistered } =
...
after(() => {
  if (prevCatalogEnv === undefined) delete process.env.CENTRICMEM_LIBRARIES_JSON;
  else process.env.CENTRICMEM_LIBRARIES_JSON = prevCatalogEnv;
  if (prevUrlEnv === undefined) delete process.env.CENTRICMEM_URL;
  else process.env.CENTRICMEM_URL = prevUrlEnv;
...
test("suggestClassify uses classify_hints from config.json", () => {
  ...
  assert.ok(suggestions.some((s: { slug: string; score: number }) => s.slug === "host" && s.score >= 3), JSON.stringify(suggestions));
});
...
test("search --tag hits YAML body_parts on corpus cards", () => {
  ...
  fs.writeFileSync(
    path.join(dir, "rec-bam10-igi-001.md"),
    [
      "---",
      "civilization: babylonian",
      "type: recipe",
  ...
  assert.ok(byPart.some((h: { file: string }) => h.file.includes("rec-bam10-igi-001")));
  const byCiv = search(paths, "", 8, { tags: ["babylonian"] });
  assert.ok(byCiv.length >= 1);
  const byColl = search(paths, "tag:bam10", 8);
  assert.ok(byColl.some((h: { file: string }) => h.file.includes("bam10")));
});
...
test("unlinked cwd writes go to unclassified, not workspace.current", () => {
  ...
  assert.strictEqual(getCurrentProjectSlug(home, code), slug);
  const starred = listProjects(home).find((p: { slug: string }) => p.slug === slug);
...
test("inbox lists files and --apply moves high-confidence", () => {
  ...
  const files = listed.filter((i: { kind: string }) => i.kind === "file");
  assert.ok(files.some((i: { relPath: string }) => i.relPath === hitRel));
  const applied = applyInbox(ws);
  assert.ok(applied.moved.some((m: { relPath: string; to: string }) => m.relPath === hitRel && m.to === slug));
  assert.ok(fs.existsSync(path.join(projectDir(ws, slug), hitRel)));
  assert.ok(applied.skipped.some((s: { relPath: string }) => s.relPath === missRel));
...
test("search --tag matches Tags field or body; tagged ranks higher", () => {
  ...
  assert.ok(tagged.some((h: { tags?: string[] }) => (h.tags ?? []).includes("deploy")));
...
test("search prefixes: type: #id; bare decision is FTS not a type filter", () => {
  ...
  assert.ok(typed.every((h: { docType: string }) => h.docType === "decision"));
  assert.ok(!typed.some((h: { docType: string }) => h.docType === "session"));

  const bare = search(paths, "decision", 10);
  assert.ok(bare.some((h: { docType: string }) => h.docType === "session"));

  const byHash = search(paths, "#0001", 10);
  assert.ok(byHash.some((h: { file: string }) => /decisions[\\/]0001-/.test(h.file)));
  const byId = search(paths, "id:0001", 10);
  assert.ok(byId.some((h: { file: string }) => /decisions[\\/]0001-/.test(h.file)));
});
...
test("search project: jumps which index; CJK tag matches Tags or body", () => {
  ...
  assert.ok(jumped.some((h: { heading: string }) => h.heading.includes("HostOnlyWidget")));
...
test("keep stores original; search hits stub; show --original returns full text", () => {
  ...
  assert.ok(hits.some((h: { file?: string }) => (h.file ?? "").includes("imported/kept/")));
  assert.ok(hits.some((h: { attach?: string }) => (h.attach ?? "").startsWith("imported/attach/")));
...
test("shouldSkipIndexFile skips secondary dumps but keeps catalogs and corpus cards", () => {
  assert.equal(
    shouldSkipIndexFile("imported/academic/secondary/chinese-medicine/kuriyama-1999/Kuriyama-Epilogue-1999.md"),
    true,
  );
  ...
  assert.equal(deduped.find((r: { file: string; workSiblings?: number }) => r.file === "b.md")?.workSiblings, 1);
});
...
test("doctor reports uninitialized hub", async () => {
  ...
  assert.ok(r.errors.some((e: string) => /uninitialized/i.test(e)));
});
...
test("looksLikeClientFolder detects CLI package, not a library hub", () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "centricmem", bin: { centricmem: "./dist/cli.js" } }),
  );
...
test("setup refuses to use the client folder as --workspace", () => {
  ...
  assert.throws(
    () => runSetup({ workspace: client, bootstrap: true, codeRoot: freshDir("t-setup-client-code") }),
    /client folder as the memory library/,
  );
});
...
test("persistProductHome pointer wins over env that points at the client", () => {
  ...
  try {
    ...
    if (prevPtr === undefined) delete process.env.CENTRICMEM_HOME_POINTER;
    else process.env.CENTRICMEM_HOME_POINTER = prevPtr;
    if (prevHome === undefined) delete process.env.CENTRICMEM_HOME;
    else process.env.CENTRICMEM_HOME = prevHome;
    if (prevWs === undefined) delete process.env.CENTRICMEM_WORKSPACE;
    else process.env.CENTRICMEM_WORKSPACE = prevWs;
