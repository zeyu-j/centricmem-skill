...
const { isLibrarianGuest, librarianGuestOrigin, guestHubWriteMessage } = await import(
  toImport(path.join(distDir, "guest.js"))
);
...
function writeCatalog(file: string, extra: Record<string, unknown> = {}): void {
  fs.writeFileSync(
    file,
    JSON.stringify({
      version: 1,
...
test("guest doctor lists catalog libraries, not leftover hub slugs", async () => {
  ...
  const ws = JSON.parse(fs.readFileSync(wsFile, "utf8")) as {
    projects: Record<string, { path: string; linked_at: string }>;
  };
  ...
  assert.deepEqual(r.libraries.map((row: { id: string }) => row.id), ["Academic"]);
  assert.ok(!r.libraries.some((row: { id: string }) => row.id === "ancient-medicine"));
  assert.ok(r.warnings.some((w: string) => /leftover CENTRICMEM_HOME is ignored for writes/i.test(w)));
  writeCatalog(process.env.CENTRICMEM_LIBRARIES_JSON!);
});
...
test("guest setup bootstrap is refused", () => {
  ...
  assert.throws(
    () => runSetup({ workspace: home, codeRoot, bootstrap: true }),
    /leftover hub/,
  );
  delete process.env.CENTRICMEM_URL;
});
