...
const {
  abortSweep,
  commitSweep,
  DEFAULT_SWEEPS_PER_WINDOW,
  openSweepDb,
  remainingSweeps,
  reserveSweep,
  SWEEP_WINDOW_MS,
} = await import(toImport(path.join(distDir, "sweep.js")));
...
test("fourth reserve in the window is denied; empty commit does not count", () => {
  ...
  for (let i = 0; i < DEFAULT_SWEEPS_PER_WINDOW; i++) {
    ...
    if (r.ok) ids.push(r.reservationId);
  }
...
test("keys on the same library share the cap; another library does not", () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    assert.equal(r.ok, true);
    if (r.ok) commitSweep(db, r.reservationId, [`ext-${i}`], { now: t0 + i });
  }
...
test("abort releases the slot; window expiry frees counted sweeps", () => {
  ...
  if (held.ok) abortSweep(db, held.reservationId);
  const again = reserveSweep(db, "lib-c", { now: t0 + 1 });
  assert.equal(again.ok, true);
  if (again.ok) commitSweep(db, again.reservationId, ["x"], { now: t0 + 2 });
