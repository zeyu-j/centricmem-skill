...
# CentricMem Agent Skill v0.21.20
...
2. If those tools are **missing**, or this chat is a pairing key and the owner needs every vault, or they need to add this librarian to the agent: **send the authenticate link**. Never ask them to paste the key here. Never one-click install. Never copy JSON into chat. Do not curl. Do not invent a hub. Keep working in the agent’s own memory.
   - **This computer is the human’s** (their browser can hit 127.0.0.1): start `centricmem connect --no-open` in the background (port 23182; ignore already running). Send [Authenticate CentricMem](https://centricmem.com/connect).
   - **This computer is remote** (Cloud Agent, VPS, headless): run `centricmem connect --device`. Send **only** the printed URL (`/connect?device=…`). Never the device secret. They have ten minutes to enter the key. Then a new chat.
   They enter **the key they want** on that page (default = every library, or a pairing key = that library).
...
3. none fit → account key: `cm_library` `{id}` (slug like `my-project`) then file there. Pairing key, or `cm_library` missing: send the authenticate link (step 0.2 — loopback `/connect` on this computer, or `centricmem connect --device` when remote) so they can enter a key on that page (default to mint/manage every vault, or that library’s pairing key); **hold the sweep in agent memory**. Do not write `unclassified`.
